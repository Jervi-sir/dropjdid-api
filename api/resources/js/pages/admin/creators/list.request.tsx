...
import {
    ...
    index,
    reject,
} from '@/actions/App/Http/Controllers/Admin/Creators/ListRequestController';
...
interface Props {
    requests: PaginatedData<CreatorRequestItem>;
    filters: {
        status: string;
        search: string;
    };
...
export default function CreatorRequestsPage({
    ...
    const handleApprove = (item: CreatorRequestItem) => {
        if (
            confirm(
                `Are you sure you want to approve creator request for "${item.user?.full_name || item.user?.username || item.phone_number}"?`,
            )
        ) {
    ...
    const handleReject = (item: CreatorRequestItem) => {
        if (
            confirm(
                `Are you sure you want to reject creator request for "${item.user?.full_name || item.user?.username || item.phone_number}"?`,
            )
        ) {
            setProcessingId(item.id);
            router.post(
    ...
    return (
        <>
            <Head title="Creator Join Requests - Admin" />
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                            Review and manage user requests to join as content
                            creators on DropJdid.
                        </p>
                    </div>
                                            ...
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p className="max-w-sm text-xs text-muted-foreground">
                                                    {filters.search
                                                        ...
                                                        : 'There are currently no creator requests in this category.'}
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
