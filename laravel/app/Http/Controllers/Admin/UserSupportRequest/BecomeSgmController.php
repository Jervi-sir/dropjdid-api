...
{
    /**
     * Display a listing of become-sgm requests.
     */
    public function index(Request $request): Response
    {
        ...
        // Compute high-level stats for SGM requests
        $stats = [
            'total' => UserSupportRequest::where('target', UserSupportRequest::TARGET_BECOME_SGM)->count(),
            'pending' => UserSupportRequest::where('target', UserSupportRequest::TARGET_BECOME_SGM)->where('status', UserSupportRequest::STATUS_PENDING)->count(),
            'approved' => UserSupportRequest::where('target', UserSupportRequest::TARGET_BECOME_SGM)->where('status', UserSupportRequest::STATUS_APPROVED)->count(),
            'rejected' => UserSupportRequest::where('target', UserSupportRequest::TARGET_BECOME_SGM)->where('status', UserSupportRequest::STATUS_REJECTED)->count(),
        ];

        return Inertia::render('admin/list-to-approve/approve-sgm', [
    ...
    /**
     * Show details of a single SGM request.
     */
    ...
    /**
     * Approve the become-sgm request.
     */
    public function approve(UserSupportRequest $supportRequest): RedirectResponse
    {
        ...
        $user = $supportRequest->user;
        if ($user) {
            $role = Role::where('code', Role::SGM)->first();
            if ($role) {
                $user->roles()->syncWithoutDetaching([$role->id]);
            }
        }

        return back()->with('success', 'SGM request approved and role assigned.');
    }

    /**
     * Reject the become-sgm request.
     */
    public function reject(Request $request, UserSupportRequest $supportRequest): RedirectResponse
    {
        ...
        return back()->with('success', 'SGM request rejected.');
    }
}
