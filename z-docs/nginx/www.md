...
1. Write the port 80 portion of the configuration to Nginx:
   ```bash
   sudo nano /etc/nginx/sites-available/www.dropjdid.com
   ```
2. Paste the HTTP server block:
   ```nginx
server {
    listen 80;
    server_name www.dropjdid.com;
...
3. Enable the site and restart Nginx:
   ```bash
   sudo ln -sf /etc/nginx/sites-available/www.dropjdid.com /etc/nginx/sites-enabled/
   sudo nginx -t
...
```bash
sudo certbot --nginx -d www.dropjdid.com
```
